...
#define UILIST_MAX_COLUMNS 32
	...
	public:
		...
		virtual bool SetSelectMark(int iIndex) = 0; //设置选中状态，但是不发送选中事件
	...
	public:
		...
		virtual bool Select(bool bSelect = true, bool bNotifyOwner = true) = 0;
	...
	{
	public:
/*    CListUI(LPCTSTR pstrListBodyUIName = _T("CListBodyUI"));*/
		CListUI();
		...
		bool SetSelectMark(int iIndex) override; //设置选中状态，但是不发送选中事件
	...
	public://list支持多选
		...
		int GetNextSelect(int nIndex) ;   //获取下一个选择的行		
		int GetSelectCount() ; //获取当前选择的个数
		bool IsEnsureNeed();
	...
	protected:
		...
		std::vector<int> m_listSel; //list支持多选
		std::mutex m_listSelMu;


		int m_iExpandedItem;
		IListCallbackUI* m_pCallback;
		CListBodyUI/*CListBodyUIBase*/* m_pList;
		CListHeaderUI* m_pHeader;
		TListInfoUI m_ListInfo;
		bool bEnsureNeed_ = false;
	};
...
class UILIB_API CListHeaderItemUI : public CContainerUI
	...
	bool Select(bool bSelect = true, bool bNotifyOwner = true) override;
